...
export default function DataStatusPage() {
  ...
  const latestArtifact = [summary.metadata.retrievedAt, stars.metadata.retrievedAt, cpdReports.generatedAt, population.generatedAt, demographics.generatedAt, elections.metadata.generatedAt, budget.metadata.generatedAt, actuals.metadata.generatedAt, initiatives.metadata.generatedAt, transition.generatedAt, unmapped.generatedAt, historical.metadata.generatedAt, validationSummary.generatedAt].sort().at(-1)!;
  ...
  const rows = [
    ...
    ["Neighborhood ACS profiles", "2016–2020 ACS 5-year", demographics.status, dateOnly(demographics.generatedAt), `${demographics.neighborhoodsWithAcs}/${demographics.snaRegions} map regions with estimate/MOE pairs; ${demographics.automatedTextProfiles} text-extracted and ${demographics.manuallyTranscribedProfiles.length} image-only profiles independently transcribed`],
