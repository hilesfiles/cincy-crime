...
export type HistoricalNeighborhood = { id: string; slug: string; name: string; sourceName: string; members: string[]; counts: Counts; population: number | null; populationYear: number; populationMarginOfError: number | null; populationMethod: string; rates: { violentPer1000: number | null } };
export type HistoricalPeriod = { year: number; periodType: "calendar_year" | "same_date_ytd"; start: string; end: string; status: string; sourceSystems: string[]; sourceGrain: string; city: Counts; unassigned: Counts; populationYear: number; cityPopulation: number; populationMarginOfError: number | null; populationMethod: string; rates: { violentPer1000: number | null }; reconciliation: { status: string; rule: string }; unmappedNeighborhoods: Array<{ label: string; count: number }>; unmappedOffenses: Array<{ label: string; count: number }>; neighborhoods: HistoricalNeighborhood[] };
...
export function annualAsCrimeSummary(historical: HistoricalData, current: CrimeSummary, year: number): CrimeSummary {
  ...
  const neighborhoods = period.neighborhoods.map((row) => {
    ...
    return {
      ...
      currentYtd: row.counts, priorYtd: priorCounts, current28: emptyCounts(), previous28: emptyCounts(), population: row.population, populationYear: row.populationYear, populationMarginOfError: row.populationMarginOfError, populationMethod: row.populationMethod, rates: { violentYtdPer1000: row.rates.violentPer1000 },
  ...
  return {
    ...
    city: { currentYtd: period.city, priorYtd: priorCity, current28: emptyCounts(), previous28: emptyCounts(), population: period.cityPopulation, populationMarginOfError: period.populationMarginOfError, populationMethod: period.populationMethod, rates: { violentYtdPer1000: period.rates.violentPer1000 } },
    neighborhoods,
  };
}
