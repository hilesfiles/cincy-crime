...
export function ExplorerDashboard({ mapData, summary, starsSummary }: { mapData: MapData; summary: CrimeSummary; starsSummary: CrimeSummary }) {
  ...
  const values = useMemo(() => Object.fromEntries(summary.neighborhoods.map((row) => [row.sourceName,
    metric === "violent" ? row.currentYtd.violent : metric === "violent-rate" ? row.rates?.violentYtdPer1000 ?? null : metric === "property" ? row.currentYtd.property : metric === "part1" ? row.currentYtd.totalPart1 : metric === "violent-change" ? changeValue(row.changes.violentYtd) : changeValue(row.changes.violent28),
    ...
    <section className="mx-auto max-w-[1480px] px-5 py-5 lg:px-9 lg:py-7">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.62fr)_minmax(340px,0.78fr)]">
        ...
        <aside className="flex flex-col gap-4">
          {selectedRow ? <div className="border-t-4 border-[#e07b36] bg-white p-5 shadow-sm"><p className="eyebrow">Selected area</p><h2 className="mt-2 text-2xl font-black text-[#143a4a]">{selectedRow.name}</h2><div className="mt-5 grid grid-cols-2 gap-3"><Kpi label="Violent YTD" value={selectedRow.currentYtd.violent} sub={`${selectedRow.priorYtd.violent} prior`} /><Kpi label="Per 1,000" value={selectedRow.rates?.violentYtdPer1000?.toFixed(1) ?? "Unavailable"} sub={`${selectedRow.population?.toLocaleString() ?? "No"} population (${selectedRow.populationYear ?? "—"})`} /><Kpi label="Latest 28 days" value={selectedRow.current28.violent} sub={`${selectedRow.previous28.violent} previous`} /><Kpi label="28-day change" value={formatChange(selectedRow.changes.violent28)} sub={`${selectedRow.current28.violent - selectedRow.previous28.violent >= 0 ? "+" : ""}${selectedRow.current28.violent - selectedRow.previous28.violent} offenses`} /></div></div> : <div className="border-t-4 border-[#0a766e] bg-white p-5 shadow-sm"><p className="eyebrow">Citywide snapshot</p><h2 className="mt-2 text-xl font-black text-[#143a4a]">Reported Part I offenses</h2><p className="mt-2 text-sm leading-6 text-[#5b6d73]">Select a map area for counts, change, and the 2020-population violent-crime rate.</p></div>}
          <div className="grid grid-cols-2 gap-3"><Kpi label="City violent YTD" value={summary.city.currentYtd.violent} sub={`${summary.city.priorYtd.violent} prior YTD`} /><Kpi label="City per 1,000" value={summary.city.rates?.violentYtdPer1000?.toFixed(1) ?? "Unavailable"} sub="2020 population denominator" /><Kpi label="Latest 28 days" value={summary.city.current28.violent} sub={`${summary.city.previous28.violent} previous`} /><Kpi label="28-day change" value={formatChange(cityRecentChange)} sub={`${summary.city.current28.violent - summary.city.previous28.violent >= 0 ? "+" : ""}${summary.city.current28.violent - summary.city.previous28.violent} offenses`} /></div>
          <div className="border border-[#d1dcdb] bg-white p-5"><p className="eyebrow">Two-layer provenance</p><p className="mt-2 text-sm font-bold text-[#143a4a]">Fresher aggregate: CPD neighborhood reports</p><p className="mt-1 text-xs leading-5 text-[#607278]">Preliminary aggregate counts through <strong>{summary.metadata.cutoff}</strong>; these drive the map, comparisons, and rates.</p><p className="mt-3 text-sm font-bold text-[#143a4a]">Offense detail: STARS</p><p className="mt-1 text-xs leading-5 text-[#607278]">Offense-level rows through <strong>{starsSummary.metadata.cutoff}</strong>, preserved separately and not presented as equally fresh.</p><p className="mt-3 text-xs leading-5 text-[#607278]">Rates use official 2020 City Planning neighborhood profiles.</p></div>
        </aside>
      </div>
    </section>
  );
