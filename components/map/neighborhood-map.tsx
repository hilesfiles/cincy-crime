...
import { crimeLegendSteps, signedChangeColor } from "@/lib/map-colors";
...
export function NeighborhoodMap({ data, metricValues, colorValues = metricValues, metricLabel = "Selected metric", changeLabel = "Change from comparable prior period", formatValue = (value) => value === "new-activity" ? "New activity" : value?.toLocaleString() ?? "Unavailable", formatChange = (value) => value === "new-activity" ? "New activity" : value === null ? "Unavailable" : `${value > 0 ? "+" : ""}${value.toFixed(1)}%`, profileQuery = "", onSelect }: { data: MapData; metricValues?: Record<string, MapMetricValue>; colorValues?: Record<string, MapMetricValue>; metricLabel?: string; changeLabel?: string; formatValue?: (value: MapMetricValue) => string; formatChange?: (value: MapMetricValue) => string; profileQuery?: string; onSelect?: (region: MapRegion) => void }) {
  ...
  const colorFor = (region: MapRegion) => signedChangeColor(colorValues?.[region.sourceName] ?? null);
  return (
    <div className="relative min-h-[480px] overflow-hidden rounded-sm bg-[#eef3f2] lg:min-h-[650px]">
      <div className="absolute left-1/2 top-4 z-10 w-[min(940px,calc(100%-2rem))] -translate-x-1/2 rounded-sm border border-[#d0dcda] bg-white/95 px-3 py-2 shadow-sm" aria-label="Change-rate color legend"><p className="text-center text-[0.6rem] font-black uppercase tracking-[0.09em] text-[#536970]">Decrease ← signed change (%) → increase</p><div className="mt-1 grid grid-cols-[repeat(17,minmax(0,1fr))] gap-px">{crimeLegendSteps.map((step, index) => <div key={`${step.label}-${index}`} className="min-w-0 text-center"><span className="block h-3" style={{ backgroundColor: step.color }} aria-hidden="true"/><span className="mt-0.5 block text-[0.48rem] font-bold text-[#536970] tabular sm:text-[0.56rem]">{index < 8 ? `−${step.label}` : index > 8 ? `+${step.label}` : step.label}</span></div>)}</div><p className="mt-1 text-center text-[0.56rem] text-[#68797e]">Only exact 0% is gray; hatching is unavailable. New activity uses strongest red. {availableChanges} areas have a comparison.</p></div>
